...
| **Tighten workflow security lint policy** | `zizmor` is intentionally advisory/non-blocking to avoid release friction during foundational CI/release work. Next step is optimizing install/runtime and then moving findings from informational signal to enforceable policy. | Revisit after initial advisory period and findings triage |
